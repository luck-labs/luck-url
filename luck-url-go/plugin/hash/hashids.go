...
var sepsOriginal = []int32("cfhistuCFHISTU")
...
func NewWithData(data *HashIDData) (*HashID, error) {
	...
	uniqueCheck := make(map[int32]bool, len(data.Alphabet))
	for _, a := range data.Alphabet {
		if _, found := uniqueCheck[a]; found {
			return nil, fmt.Errorf("duplicate character in alphabet: %s", string([]int32{a}))
		}
		uniqueCheck[a] = true
	}

	alphabet := []int32(data.Alphabet)
	salt := []int32(data.Salt)

	seps := duplicateint32Slice(sepsOriginal)
...
func (h *HashID) EncodeInt64(numbers []int64) (string, error) {
	...
	alphabet := duplicateint32Slice(h.alphabet)
	...
	maxint32Length := h.maxLengthPerNumber * len(numbers)
	if maxint32Length < h.minLength {
		maxint32Length = h.minLength
	}

	result := make([]int32, 0, maxint32Length)
	lottery := alphabet[numbersHash%int64(len(alphabet))]
	result = append(result, lottery)
	hashBuf := make([]int32, maxint32Length)
	buffer := make([]int32, len(alphabet)+len(h.salt)+1)
	...
	if len(result) < h.minLength {
		...
		result = append([]int32{h.guards[guardIndex]}, result...)
	...
	for len(result) < h.minLength {
		consistentShuffleInPlace(alphabet, duplicateint32Slice(alphabet))
...
func (h *HashID) DecodeInt64WithError(hash string) ([]int64, error) {
	hashes := splitint32s([]int32(hash), h.guards)
	hashIndex := 0
	...
	if len(hashBreakdown) > 0 {
		...
		hashes = splitint32s(hashBreakdown, h.seps)
		alphabet := duplicateint32Slice(h.alphabet)
		buffer := make([]int32, len(alphabet)+len(h.salt)+1)
...
func splitint32s(input, seps []int32) [][]int32 {
	splitIndices := make([]int, 0)
	for i, inputint32 := range input {
		for _, sepsint32 := range seps {
			if inputint32 == sepsint32 {
				splitIndices = append(splitIndices, i)
			}
		}
	}

	result := make([][]int32, 0, len(splitIndices)+1)
	inputLeft := input[:]
...
func hash(input int64, alphabet []int32, result []int32) []int32 {
	result = result[:0]
	for {
...
func unhash(input, alphabet []int32) (int64, error) {
	result := int64(0)
	for _, inputint32 := range input {
		alphabetPos := -1
		for pos, alphabetint32 := range alphabet {
			if inputint32 == alphabetint32 {
				alphabetPos = pos
				break
			}
		}
...
func consistentShuffle(alphabet, salt []int32) []int32 {
	if len(salt) == 0 {
		return alphabet
	}

	result := duplicateint32Slice(alphabet)
...
func consistentShuffleInPlace(alphabet []int32, salt []int32) {
	if len(salt) == 0 {
		return
	}
...
func duplicateint32Slice(data []int32) []int32 {
