...
import (
	...
	"github.com/luck-labs/luck-url/common/consts/rpc_const"
 ...
 */
func Init() {
	rpcServerConfig := conf.GlobalConfig.RpcServer
	addr := rpcServerConfig.Address
	RPCClientPool = NewClientPool()
	// 数据结构注册
	gob.Register(retcode.Success)
	gob.Register(retcode.Failed)
	gob.Register(retcode.ErrJsonMarshal)
	gob.Register(retcode.ErrJsonUnmarshal)
	gob.Register(retcode.ErrRedisReadFail)
	gob.Register(retcode.ErrRedisWriteFail)
	srv := NewServer(addr)
	// 方法注册
	srv.Register(rpc_const.MethodCreateUrl, rpcservice.CreateUrl) // 创建短链
	srv.Register(rpc_const.MethodGetUrl, rpcservice.GetUrl)       // 获取短链
	go srv.Run()
}
