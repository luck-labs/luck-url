...
var BitLenTime int      // bit length of time
var BitLenSequence int  // bit length of sequence number
var BitLenMachineID int // bit length of machine id

func Init() {
	snowflakeConf := conf.GlobalConfig.Snowflake
	BitLenTime = snowflakeConf.TimeLength
	BitLenSequence = snowflakeConf.SequenceLength
	BitLenMachineID = snowflakeConf.MachineLength
...
func Decompose(id uint64) map[string]uint64 {
	maskSequence := uint64((1<<BitLenSequence - 1) << BitLenMachineID)
...
func GetNextId(ctx context.Context) int64 {
	id, err := sf.NextID()
	if err != nil {
		log.Error(ctx, log.MduSnowflake, log.IdxSnowflakeGenId, logrus.Fields{
			"err": err,
		})
	}
	return int64(id)
